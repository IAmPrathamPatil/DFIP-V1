...
slice. There is no P12, P13, or P14 in this repository. Deferred architecture
is **V2**.
...
- **P11 native PivotTables.** The nine Daily Report sheets are real Excel
  ...
  expand/collapse) bound to PublishedFacts. P12/P13/P14 are not started.
- **Working-set / publication boundary.** Admin `/admin/facts` → `GET /facts`.
  Client portal and `PublishedFacts.m` → `GET /publications/current/facts`.
...
- **Known V1 limitations.** In-memory stores; no HTTP upload; SQL migrations
  are not a live database; `dev_token` is development/test only; password
  sign-in issues HS256 JWTs from `app_user` membership (not a hosted IdP);
  application-level `client_id` scoping is not RLS; V1 is not production
  tenant isolated.
...
- No P12 / P13 / P14. No August republish. No publication_current move

## V1 limitations (expected)

- Persistence is in-memory. Restarting the API process loses working-set and
  publication state.
- P3 ingestion and P4 transformation are libraries, not an HTTP upload
  pipeline.
- SQL under `supabase/migrations/` is the schema artifact. The running API
  does not open PostgreSQL.
...
- Durable private source-file **upload custody** is implemented: originals are
  ...
  Empty `DFIP_STORAGE_ENDPOINT` is in-memory (not durable across process
  restart). Set it to a private local directory for durable custody. Do not put
  blobs in PostgreSQL. Do not ship storage credentials to Excel or the SPA.
