-- Permanent company delete for authorized publishers after deactivation.
-- SECURITY DEFINER so dfip_api can remove client-owned rows without table DELETE
-- grants on client / membership / catalog / audit. Every statement is
-- client_id-scoped. No CASCADE.
...
COMMENT ON FUNCTION dfip_delete_company(uuid) IS
    'Permanently delete one inactive company and its tenant-scoped rows. No CASCADE.';
